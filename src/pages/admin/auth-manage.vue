    ...
    <el-table :data="tableData" size="mini">
      ...
      <el-table-column prop="status" label="当前状态" align="center">
        <template slot-scope="scope">
          ...
          <span v-else-if="scope.row.status === 1" style="color:#00ca00"
            >认证通过</span
          >
          <span v-else-if="scope.row.status === 2" style="color:#5a99f7"
            >智能审核通过</span
          >
          <span v-else-if="scope.row.status === 3" style="color:#ff6363"
            >认证失败</span
          >
          <span v-else>未知</span>
        </template>
          ...
          <el-popover placement="top" width="160" v-model="popoverVisible">
            <div style="text-align: center; margin: 0">
              <el-button
                size="mini"
                type="danger"
                @click="handlePass(scope.row, 'reject')"
                >拒绝</el-button
              >
              <el-button
                type="primary"
                size="mini"
                @click="handlePass(scope.row, 'pass')"
                >通过</el-button
              >
            </div>
            <el-button
