<template>
  <div>
    <el-card
      ...
      :class="[
        this.select ? 'box-card-select' : '',
        this.$scopedSlots.content ? 'box-card-self' : 'aa',
      ]"
      class="box-card"
    >
    ...
    <el-card
      v-else
      :class="this.select ? 'box-card-select' : ''"
      class="box-card"
    >
      <span
        ...
        >{{ this.num }} 开心果</span
      >
      <span :class="this.select ? 'box-card-text-select' : ''"
        >￥{{ this.num / 10 }}.00</span
      >
    </el-card>
  </div>
...
.box-card:hover {
  border: 1px solid rgba(125, 171, 239, 0.565);
}
